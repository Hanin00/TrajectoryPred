...
def onePickleProcessing(fileName, df, frameNum, defSeqNum) : 
    ...
    for carIdx in range(len(idList)):  #
        ...
        # [frame, x, y] 저장
        posList = []  
        [posList.append([filledDf['x'].iloc[sampleIdx*6], filledDf['y'].iloc[sampleIdx*6]]) for sampleIdx in range(defSeqNum)]
        list_of_xy.append(posList)
    
    list_Df = pd.DataFrame(list_of_xy, columns = ['xyPos'])
